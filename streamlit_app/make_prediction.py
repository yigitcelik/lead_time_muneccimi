...
class get_data():
    def get_predict(df):
        ...
        #build features
        card_col= pd.read_pickle("/app/lead_time_muneccimi/streamlit_app/highcardinal_columns.pickle")
        ...
        scaler = pickle.load(open('/app/lead_time_muneccimi/streamlit_app/neural_network(mae 9.1)_scaler.pkl', 'rb'))
        X = scaler.transform(data.drop(['is_emri'],axis=1))

        model =  pickle.load(open('/app/lead_time_muneccimi/streamlit_app/neural_network(mae 9.1).pkl', 'rb'))
